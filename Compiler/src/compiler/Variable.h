...
struct VariableInfo {
    ...
    DataType Type() const { return Pointer.Type; }
    ...
    bool IsField() const { return !ParentClass.empty() || (Flags & vfField); }

    int Address() const { return Pointer.Address; }

    VarScopeType Scope() const { return Pointer.Scope; }

    bool IsHeadMemberOf(const string &instance) const;

    bool IsClassHead() const;

    bool Match(const string &name, const string &parent);
};
