...
void Disassembler::OutputLine(const std::string &line)
{
...
std::string Disassembler::ReadInstruction()
{
    std::string instr;
    std::string desc;
...
std::string Disassembler::WriteFunctionHeader(int id, u8 returnType, u8 argCount)
{
    std::string func = "<<< Function [" + STRING(id) + "] " + "(" + STRING(argCount) + ") " + ": " + STRING(returnType) + " >>>";
    return func;
}

std::string Disassembler::WriteInstruction(size_t addr,
                                      const std::string &op,
...
{
    std::string str = std::format("{:6}", addr) + ":";
...
std::string Disassembler::ReadHex(int count)
{
    std::string line;
...
std::string Disassembler::ReadString()
{
    std::string str = "\"";
    do {
...
void Disassembler::Disassemble()
{
    ...
    while (m_Pos < m_ConstantsPos) {
        std::string line = ReadInstruction();
        OutputLine(line);
    }
    OutputLine(divider);
    OutputLine("    ");
    ...
    while (m_Pos < m_StringsPos) {
        std::string c = std::format("{:4}", constId++) + ":";
        ALIGN_STRING(c, 8);
        c += ReadHex(4);
    ...
    while (m_Pos < m_Length) {
        std::string str = std::format("{:4}", stringId++) + ":";
        ALIGN_STRING(str, 8);
        str += ReadString();
        OutputLine(str);
    }
